...
| Compliance export downloads broken / serves HTML | `sw.js` `API_RE` must include `/export` (fixed since `camp-v4`; current cache is **`camp-v12`**). If the workbook itself 500s, see the audit-workbook row below (illegal sheet name). |
...
| **Browser console shows a CSP violation / a resource silently fails to load** | `public/index.html` `<head>` `Content-Security-Policy` meta tag — a new external resource (font, script, API host) needs an allowlist entry there, not a workaround. Bump `sw.js` `CACHE` (currently `camp-v9`) whenever `index.html` changes. |
